...
def test_response_templates_cover_10_languages():
    ...
    for intent in VoiceIntent:
        ...
        for lang in SUPPORTED_LANGUAGES:
            assert lang in intent_templates, (
                f"Intent {intent.name} missing template for language: {lang}"
            )
            assert len(intent_templates[lang]) > 0, (
                f"Template empty for {intent.name} in lang {lang}"
            )
...
def test_new_intent_patterns_detected_in_multiple_languages():
    ...
    cases = [
        ...
        ),  # 'સાપ્તાહિક' = weekly, unique keyword
    ]
...
def test_detect_language_from_transliterated_kannada():
    """Transliterated Kannada should resolve through the shared detector."""
    assert (
        VoiceEntityExtractor.detect_language_from_text("Tomato bele yaavaga Kolar mandi alli?")
        == "kn"
    )
