...
class BaseAgent(ABC):
    ...
    def _build_memory_context(self, context: Optional[dict]) -> str:
        ...
        Converts the session's structured entities (commodity, quantity,
        district, price_per_kg, current_agent) into a plain-text block that
        is injected into the LLM prompt as a 'memory' message so the model
        always knows what the conversation is actually about.

        Args:
            context: Context dict from SupervisorAgent.process_with_session()
                     Keys used: entities, conversation_summary, current_agent

        Returns:
            Formatted memory string, or empty string if no useful context.
        """
        if not context:
            return ""
    ...
    async def generate_with_llm(
        ...
        temperature: Optional[float] = None,
        max_tokens: Optional[int] = None,
        context: Optional[dict] = None,  # * NEW — Phase 4 memory injection
    ) -> str:
        """
        Generate response using LLM.

        Automatically injects memory context (entities, conversation summary,
        previous agent) as a system message before the user message so every
        LLM call has access to structured session facts without requiring
        ...
        Args:
            ...
            temperature: Override temperature
            max_tokens: Override max tokens
            context: Optional context dict — if provided, extracts memory block
