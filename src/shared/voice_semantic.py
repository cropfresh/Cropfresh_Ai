"""Shared semantic endpointing helpers layered on top of acoustic VAD."""
...
FILLER_PATTERNS = {
    ...
    "kn": {"ಒಂದು ನಿಮಿಷ", "ಸ್ವಲ್ಪ", "ಸ್ವಲ್ಪ ತಾಳಿ", "ಅಯ್ಯೋ", "ಹಂ"},
...
class SemanticLLMMessage(BaseModel):
    """Minimal message model compatible with the provider-layer generate contract."""
...
class SupportsGenerate(Protocol):
    ...
    async def generate(self, messages: list[SemanticLLMMessage], **kwargs): ...
...
async def evaluate_semantic_flush(
    *,
    ...
    try:
        response = await asyncio.wait_for(
            llm_provider.generate(
                [SemanticLLMMessage(role="user", content=prompt)],
                temperature=0.0,
                max_tokens=2,
            ),
    ...
    except Exception as exc:  # noqa: BLE001 - fall back to acoustic flush on provider failure
